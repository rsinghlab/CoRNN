...
def linear_regression_cross_validation(all_data, cells, args):
...
def new_lr_model(select_dict, args):
	if args.use_sgd:
		clf = SGDRegressor(
			loss = select_dict['loss'],
			penalty = select_dict['penalty'],
			n_iter = select_dict['n_iter'],
        	alpha = select_dict['alpha'],
        	random_state=0,\
			verbose=1)
	else:
	...
	for target in targets:
		...
		if args.use_sgd:
			for idx, param_selection in enumerate(list(itertools.product(*hyperparameter.values()))):
				...
				clf, valid_auc, model_test_auc= SGDRegressor_cross_validation(all_data,select_dict,cells,args)
				if valid_auc > best_auc:
					best_auc = valid_auc
					best_param = select_dict

					filename = '{}_sgd.sav'.format(target)
			...
			print("~~~~~SGD Grid search of {}: {}~~~~~~".format(target,grid_search_results[target]["test_auc"]))

		else:
			clf, valid_auc, model_test_auc= linear_regression_cross_validation(all_data,cells,args)
			filename = '{}_lr.sav'.format(target)
			file_save_path = os.path.join(run_dpath,filename)
			pickle.dump(clf, open(file_save_path, 'wb'))
