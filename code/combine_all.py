...
def test(model,test_loader,task):
	...
	for batch in tqdm(test_loader):
		...
		y_pred = y_pred[0,:].detach().tolist()
		# print(y_pred)
		y_pred_prob = torch.tensor(y_pred)
		softmax_func = nn.Softmax(dim=0)
		y_pred_prob = softmax_func(y_pred_prob)
		# print(y_pred_prob)
		# exit()
		cur_pred += [y_pred_prob.detach().tolist()]
		label = 0 if y_pred_prob[0]>y_pred_prob[1] else 1
		cur_labels += [label]
		# print(cur_pred)
		# print(cur_labels)	#calculate accuracy
	# pred_binary = [to_binary(val) for val in cur_pred]
	
	return cur_pred,cur_labels
...
def load_model_and_run(model_path,test_loader):
	...
	pred_prob, pred_labels = test(model,test_loader,"cla")

	return pred_prob, pred_labels

def prepare_data_cross_validation_with_mean_evec_and_region():
	...
	save_dir = os.path.join("data","100kb_test_by_chr")
	...
	for key, data in cleaned_data.items():
		cell = key.split("_")[0]
		chr_num = key.split("_")[1]
		test_dataset = abDataset_with_mean_and_region(data)
		test_loader = DataLoader(test_dataset,batch_size=1, shuffle=False)
		data_save_dir = os.path.join(save_dir,cell)
		torch.save(test_loader,os.path.join(data_save_dir,"{}_test.pth".format(chr_num)))
		print("save file: ",os.path.join(data_save_dir,"{}_test.pth".format(chr_num)))
...
def main():
	...
	# prepare_data_cross_validation_with_mean_evec_and_region()

	test_save_dir = os.path.join("data","combined_test_results_2021_11_17")
	...
	for cell in cells:
		...
		for chrom in range(1,23):
			print(cell,chrom)
			save_file = os.path.join(cell_save_dir,"chr_{}_test.csv".format(chrom))
			test_loader = torch.load(os.path.join(test_data_dir,"chr{}_test.pth".format(chrom)))
			pred_prob, pred_labels = load_model_and_run(model_path,test_loader)

			with open(save_file,"w", newline='') as csvfile:
				writer = csv.writer(csvfile)
				writer.writerow(["cell","chrom","start","end","eigenvector","cla_label","mean_evec","region","A prob","B prob","pred label"])
				...
				for idx, batch in enumerate(test_loader):
					...
					row.append(str(pred_prob[idx][1]))
					row.append(str(pred_prob[idx][0]))
					row.append(str(pred_labels[idx]))
