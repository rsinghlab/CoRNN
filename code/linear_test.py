...
def main():
	...
	if not args.use_sgd:
		if not args.add_std:
			if not args.add_mean:
				model_dir = os.path.join("data","exp_data","cla","rf","run_2021-08-11-15-40-26")
			if args.add_mean:
				model_dir = os
				.path.join("data","exp_data","cla","rf","run_2021-08-11-16-40-17")
		if args.add_std:
			if not args.add_mean:
				model_dir = os.path.join("data","exp_data","cla","rf","run_2021-08-12-16-33-47")
			if args.add_mean:
				model_dir = os.path.join("data","exp_data","cla","rf","run_2021-08-12-16-42-48")
	else:
		if not args.add_std:
			if not args.add_mean:
	...
	for target in targets:
		...
		a_logger.debug("linear regression of {}: {}~~~~~~".format(target,test_auc))
