...
def train_cross_validation(model,loss_fn,args):
	...
	for fold in range(args.num_fold):
		...
		args.fold_exp_data_path = os.path.join(args.fold_exp_data_path,"fold_{}".format(fold))
		if not os.path.exists(args.fold_exp_data_path):
			os.mkdir(args.fold_exp_data_path)
...
def valid(model, valid_loader, loss_fn, args, max_auroc,max_auprc, epoch):
	...
	if args.task == "cla":
		...
		if args.cross_validation == True:
			img_save_dir = os.path.join(args.exp_dpath,"img","fold_{}".format(args.current_fold))
...
def create_new_model(args):
	...
	if args.model == "lstm":
		...
		print(model_param)
		model = gru(args.seq_len,model_param).to(device)
	elif args.model[:3] == "cnn":
		n = args.model.split("_")[1]
...
def main():
    ...
    if args.train:
    	if args.use_allcells:
    		train_cross_validation_by_cells_allcells(model,loss_fn,args)
    	elif args.cross_validation:
    		best_model = train_cross_validation_by_cells(model,loss_fn,args)
    	else:
